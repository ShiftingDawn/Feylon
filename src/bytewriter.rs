...
pub fn write_parsed_program_to_file(input_path: &str, ctx: &linker::LinkerContext) {
    ...
    let mut out_file = std::fs::File::create(&output_file_path).unwrap_or_else(|e| {
        eprintln!(
            "ERROR: Could not open file for bytecode representation: {}",
            e
        );
        std::process::exit(1);
    });
    ...
    eprintln!(
        "SUCCESS: Written bytecode representation to: {}",
        output_file_path
    );
}
